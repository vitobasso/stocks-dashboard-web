...
function Checkbox({ className, ...props }: React.ComponentProps<typeof CheckboxPrimitive.Root>) {
  ...
  return (
    <CheckboxPrimitive.Root data-slot="checkbox" asChild {...props}>
      <span className={cn(base, stateChecked, stateIndeterminate, focusValidation, className)}>
        <CheckboxPrimitive.Indicator
          data-slot="checkbox-indicator"
          className="flex items-center justify-center text-current transition-none"
        >
          <CheckIcon className="size-3.5 group-data-[state=indeterminate]:hidden" />
          <MinusIcon className="size-3.5 hidden group-data-[state=indeterminate]:block group-data-[state=checked]:hidden" />
        </CheckboxPrimitive.Indicator>
      </span>
